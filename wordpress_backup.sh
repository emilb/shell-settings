...
wpuser='emil' #WordPress user name
wpblog='forme' #WordPress blog name (the part that appears in the domain)
wppwd='caster' #WordPress password
wphost='emibre.com' #WordPress host
...
[ "$wpblog" == "" ] && echo "Please enter your WordPress blog name:" && read wpblog;
[ "$wppwd" == "" ] && echo "Please enter your WordPress user password:" && read wppwd;
[ "$wphost" == "" ] && echo "Please enter your WordPress host name:" && read wphost;

#if blog is left blank just use the user name
[ "$wpblog" == "" ] && wpblog="$wpuser"

echo "Exporting WordPress blog: $wpblog by $wpuser at $wphost";
...
curl --cookie-jar "$cookies"\
     --output /dev/null \
     "http://${wpblog}.${wphost}/wp-login.php"
...
#log	username
#pwd	password
#redirect_to	http://blogname.${wphost}/wp-admin/
#rememberme	forever
#testcookie	1
#wp-submit	Log In
...
curl --cookie-jar "$cookies" \
     ...
     --max-redirs 0 \
	 --data "log=${wpuser}&pwd=${wppwd}&rememberme=forever&redirect_to=http%3A%2F%2F${wpblog}.${wphost}%2Fwp-admin%2F&testcookie=1&wp-submit=Log%20In" \
     "http://${wpblog}.${wphost}/wp-login.php"
...
wget --secure-protocol=auto \
     ...
     --load-cookies "$cookies" \
 	 ...
 	 --max-redirect=0\
	 "http://${wpblog}.${wphost}/wp-admin/export.php?download=true&submit=Download%20Export%20File"
...
egrep -oe "http://${wpblog}\.${wphost}[^\"]*\.((jpg)|(jpeg)|(png)|(gif)|(pdf)|(doc)|(ppt)|(odt)|(pptx)|(docx))" "$target_full" | sort | uniq > temp/files.txt
